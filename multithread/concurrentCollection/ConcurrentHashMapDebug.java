...
/**
 * 线程安全的 HashMap，比 HashTable 高效
 * JDK 1.8 源码
 */
public class ConcurrentHashMapDebug<K,V> extends AbstractMap<K,V>
    ...
    static final int MAX_ARRAY_SIZE = Integer.MAX_VALUE - 8; // 最大可能数组大小
    private static final int DEFAULT_CONCURRENCY_LEVEL = 16; // 默认并发度，没有使用，为了兼容所以保留
    private static final float LOAD_FACTOR = 0.75f; // 装载因子，通常没用，一般用 n - (n >>> 2) 计算
    static final int TREEIFY_THRESHOLD = 8; // 节点树化阈值
    static final int UNTREEIFY_THRESHOLD = 6; // 节点去树化阈值
    static final int MIN_TREEIFY_CAPACITY = 64; // 节点树化时数组的最小容量，至少是 4 * TREEIFY_THRESHOLD（avoi conflicts between resizing and treeification thresholds？）
    private static final int MIN_TRANSFER_STRIDE = 16; // 单步转移的最小值，至少是 DEFAULT_CAPACITY
    private static int RESIZE_STAMP_BITS = 16; // sizeCtl 中生成 stamp 的比特位数
    private static final int MAX_RESIZERS = (1 << (32 - RESIZE_STAMP_BITS)) - 1; // 最大帮忙扩容线程数
    private static final int RESIZE_STAMP_SHIFT = 32 - RESIZE_STAMP_BITS; // The bit shift for recording size stamp in sizeCtl.
    static final int MOVED     = -1; // 迁移完成的节点哈希值
    static final int TREEBIN   = -2; // 红黑树根节点哈希值
    static final int RESERVED  = -3; // 保留节点的哈希值
    static final int HASH_BITS = 0x7fffffff; // 将节点的哈希值最高位 32 位置为 0
    ...
    /* ---------------- Fields -------------- */
    transient volatile Node<K,V>[] table; // 节点数组，长度总是 2 的 n 次方
    private transient volatile Node<K,V>[] nextTable; // 扩容辅助数组
    private transient volatile long baseCount; // 元素个数
    ...
    public V get(Object key) {
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            (e = tabAt(tab, (n - 1) & h)) != null) { // 数组已初始化且所在节点不为空
            if ((eh = e.hash) == h) {
                if ((ek = e.key) == key || (ek != null && key.equals(ek)))
                    return e.val;
            }
            else if (eh < 0) // 当前节点为红黑树
                return (p = e.find(h, key)) != null ? p.val : null;
            while ((e = e.next) != null) { // 当前节点为链表
                if (e.hash == h &&
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        int binCount = 0; // 计算所在节点的元素个数，用来控制扩容和树化
        for (Node<K,V>[] tab = table;;) { // 自旋
            Node<K,V> f; int n, i, fh;
            if (tab == null || (n = tab.length) == 0) // 数组初始化
                tab = initTable();
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) { // 当前节点为 null
                if (casTabAt(tab, i, null,
                             new Node<K,V>(hash, key, value, null))) // cas 赋值，不需要加锁，失败后自旋重试
                    break;                   // no lock when adding to empty bin
            }
            else if ((fh = f.hash) == MOVED) // 数组正在扩容
                tab = helpTransfer(tab, f);
            else {
                V oldVal = null;
                synchronized (f) { // 加锁
                    if (tabAt(tab, i) == f) { // 再次取出节点，查看是否发生变化
                        if (fh >= 0) { // hash 大于 0，表示节点为链表
                            binCount = 1;
                        ...
                        else if (f instanceof TreeBin) { // hash 小于 0，表示节点为红黑树
                            Node<K,V> p;
                            binCount = 2; // binCount != 0 方便进入下方树化的判断
                ...
                if (binCount != 0) {
                    if (binCount >= TREEIFY_THRESHOLD) // 节点元素个数达到树化的阈值，此时可能扩张数组或将节点树化
                        treeifyBin(tab, i);
                    if (oldVal != null)
                        return oldVal;
                    break;
    ...
    public static <K> KeySetView<K,Boolean> newKeySet(int initialCapacity) {
        return new KeySetView<K,Boolean>
            (new ConcurrentHashMapDebug<K,Boolean>(initialCapacity), Boolean.TRUE);
    }
    ...
    private final void tryPresize(int size) {
        int c = (size >= (MAXIMUM_CAPACITY >>> 1)) ? MAXIMUM_CAPACITY :
            tableSizeFor(size + (size >>> 1) + 1); // 计算目标容量
        int sc;
        while ((sc = sizeCtl) >= 0) {
            Node<K,V>[] tab = table; int n;
            if (tab == null || (n = tab.length) == 0) { // 数组未初始化，putAll 时出现这种情况
                n = (sc > c) ? sc : c;
                ...
                synchronized (f) {
                    if (tabAt(tab, i) == f) {
                        ...
                        if (fh >= 0) { // 链表的迁移
                            ...
                            setTabAt(nextTab, i, ln); // 扩容前的位置为 ln
                            setTabAt(nextTab, i + n, hn); // 扩容后 i + n 为 hn
                            setTabAt(tab, i, fwd); // 把原数组的 i 节点设为已处理
                            advance = true;
                        }
                            ...
                            int lc = 0, hc = 0;
                            for (Node<K,V> e = t.first; e != null; e = e.next) { // 通过 TreeBin 里的 first 链表遍历
                                int h = e.hash;
    ...
    private final void treeifyBin(Node<K,V>[] tab, int index) {
        Node<K,V> b; int n, sc;
        if (tab != null) {
            if ((n = tab.length) < MIN_TREEIFY_CAPACITY) // 数组容量小于最小树化容量 64，则扩张数组
                tryPresize(n << 1);
            else if ((b = tabAt(tab, index)) != null && b.hash >= 0) { // 节点非空且节点是链表
                synchronized (b) { // 加锁
                    if (tabAt(tab, index) == b) { // 再次判断，防止重复
                        TreeNode<K,V> hd = null, tl = null;
                        for (Node<K,V> e = b; e != null; e = e.next) { // 将 Node 链表转化为 TreeNode 链表
                            TreeNode<K,V> p =
                        ...
                        setTabAt(tab, index, new TreeBin<K,V>(hd)); // 用 TreeNode 链表构造 TreeBin，TreeBin.root 就是我们要的红黑树
                    }
                }
            }
        }
    ...
    static class BaseIterator<K,V> extends Traverser<K,V> {
        ...
        BaseIterator(Node<K,V>[] tab, int size, int index, int limit,
                    ConcurrentHashMapDebug<K,V> map) {
    ...
    static final class EntrySpliterator<K,V> extends Traverser<K,V>
        ...
        EntrySpliterator(Node<K,V>[] tab, int size, int index, int limit,
                         long est, ConcurrentHashMapDebug<K,V> map) {
            super(tab, size, index, limit);
            this.map = map;
            this.est = est;
        }
    ...
    abstract static class CollectionView<K,V,E>
        ...
        private static final long serialVersionUID = 7249069246763182397L;
        final ConcurrentHashMapDebug<K,V> map;
        CollectionView(ConcurrentHashMapDebug<K,V> map)  { this.map = map; }
         ...
         */
        public ConcurrentHashMapDebug<K,V> getMap() { return map; }
    ...
    static final class ValuesView<K,V> extends CollectionView<K,V,V>
        ...
        private static final long serialVersionUID = 2249069246763182397L;
        ValuesView(ConcurrentHashMapDebug<K,V> map) { super(map); }
    ...
    static final class EntrySetView<K,V> extends CollectionView<K,V, Entry<K,V>>
        ...
        private static final long serialVersionUID = 2249069246763182397L;
        EntrySetView(ConcurrentHashMapDebug<K,V> map) { super(map); }
